#!/usr/bin/env python3
"""
nhk_epg_fetcher.py — NHK BS8K / BSP4K EPG → XMLTV (.xml.gz)
...
Usage:        python nhk_epg_fetcher.py [output.xml.gz]
"""
...
# Retry on transient failures: connection errors and 5xx responses.
# Back-off: 0 s → 0.5 s → 1.0 s (backoff_factor=0.5, total=3 retries).
_RETRY_POLICY = Retry(
    total=3,
    backoff_factor=0.5,
...
def make_session() -> requests.Session:
    ...
    Build a requests.Session with:
      - shared headers (User-Agent, Referer)
      - connection pool sized to MAX_WORKERS (avoids urllib3 pool warnings)
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        future_map = {
            pool.submit(fetch_day, ch_id, svc_id, area_id, d, session): ch_id
